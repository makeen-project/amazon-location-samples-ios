...
class UITests: XCTestCase {
    ...
    let sampleWebsocketURL = Bundle(for: UITestBundle.self).object(forInfoDictionaryKey: "TestWebsocketUrl") as! String
    let sampleGeofenceCollectionARN = Bundle(for: UITestBundle.self).object(forInfoDictionaryKey: "TestGeofenceArn") as! String
    ...
    func launchApp(withConfiguration: Bool = true) {
        if(withConfiguration) {
            ...
            app.launchArguments += ["-websocketUrl", sampleWebsocketURL]
            app.launchArguments += ["-geofenceCollectionArn", sampleGeofenceCollectionARN]
        }
        app.launch()
        allowPermission()
        if(withConfiguration) {
